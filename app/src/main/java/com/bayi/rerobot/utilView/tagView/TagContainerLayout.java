...
package com.bayi.rerobot.utilView.tagView;
...
import static com.bayi.rerobot.utilView.tagView.Utils.dp2px;
import static com.bayi.rerobot.utilView.tagView.Utils.sp2px;
...
public class TagContainerLayout extends ViewGroup {
     ...
     */
    private int mTagBorderColor = Color.parseColor("#ffffff");
     ...
     */
    private int mSelectedTagBackgroundColor = Color.parseColor("#ffffff");

    /**
     * TagView text color(default #FF666666)
     */
    private int mTagTextColor = Color.parseColor("#FFFFFF");
    ...
    private List<MissionActionB> missionActionList;
...
//        }
        ...
        if (missionActionList == null) {
            throw new RuntimeException("NullPointer exception!");
        }
        removeAllTags();
        if (missionActionList.size() == 0) {
            return;
        }
        for (int i = 0; i < missionActionList.size(); i++) {
            onAddTag(missionActionList.get(i), mChildViews.size());
        }
    ...
    private void onAddTag(MissionActionB missionAction, int position) {
        if (position < 0 || position > mChildViews.size()) {
            throw new RuntimeException("Illegal position!");
        }
        TagView tagView;
        if (mDefaultImageDrawableID != -1) {
            tagView = new TagView(getContext(), missionAction, mDefaultImageDrawableID, language);
        } else {
            tagView = new TagView(getContext(), missionAction, language);
        }
...
//            }
            if (mColorArrayList.size() == missionActionList.size() &&
                    mColorArrayList.get(position).length >= 4) {
                colors = mColorArrayList.get(position);
            } else {
                throw new RuntimeException("Illegal color list!");
        ...
        tagView.setTagBackgroundColor(Color.parseColor("#00ffffff"));
    ...
    public void setMissionActionList(List<MissionActionB> list) {
        missionActionList = list;
        onSetTag();
    }
     ...
     */
    public void addTag(MissionActionB missionAction) {
        addTag(missionAction, mChildViews.size());
    }
     ...
     * @param position
     */
    public void addTag(MissionActionB missionAction, int position) {
        onAddTag(missionAction, position);
        postInvalidate();
    }
     ...
     * @return
     */
    public List<MissionActionB> getMissionActionList() {
        List<MissionActionB> tmpList = new ArrayList<>();
        for (View view : mChildViews) {
            if (view instanceof TagView) {
                tmpList.add(((TagView) view).getMissionAction());
            }
        }
        return tmpList;
    }
